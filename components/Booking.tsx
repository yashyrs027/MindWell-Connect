...
const TimeSlot: React.FC<{time: string, isSelected: boolean, onSelect: () => void}> = ({time, isSelected, onSelect}) => (
    <button 
      onClick={onSelect}
      className={`px-4 py-2 rounded-lg border transition-colors ${isSelected ? 'bg-blue-500 text-white border-blue-500' : 'bg-white hover:bg-slate-100 border-slate-300'}`}
    >
      {time}
    </button>
);
...
const CounselorCard: React.FC<{counselor: Counselor, onBook: (c: Counselor) => void}> = ({ counselor, onBook }) => (
    <Card className="p-6 text-center flex flex-col items-center">
        ...
        <p className="text-slate-500 text-sm mb-3">{counselor.languages.join(', ')}</p>
        <div className="flex flex-wrap justify-center gap-2 mb-4">
            {counselor.specialties.map(spec => (
                <span key={spec} className="bg-slate-100 text-slate-700 text-xs font-medium px-2.5 py-1 rounded-full">{spec}</span>
            ))}
        </div>
        ...
        return (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                <Card className="p-8 text-center max-w-sm">
                    ...
                    <h3 className="text-2xl font-bold text-slate-800 mt-4">Appointment Confirmed!</h3>
                    <p className="text-slate-600 mt-2">Your session with {counselor.name} at {selectedTime} is booked. You'll receive a confirmation email shortly.</p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-lg" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-start mb-4">
                    <div>
                        <h2 className="text-2xl font-bold text-slate-800">Book with {counselor.name}</h2>
                        <p className="text-slate-500">Select an available time slot for today.</p>
                    </div>
    ...
    return (
        <div className="space-y-6">
            <div className="text-center">
                <h1 className="text-3xl font-bold text-slate-800">Counselor Booking</h1>
                <p className="mt-2 text-slate-600">Connect with our team of professional counselors. All sessions are confidential.</p>
            </div>
